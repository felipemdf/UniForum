<template>
        ...
        <div class="flex justify-between mb-4">
          ...
          <div class="flex flex-wrap items-center gap-1">
            <button class="inline-flex items-center gap-1">
              ...
              <p class="text-sm font-medium text-gray-900">{{ topicStore.topic.username }}</p>
            </button>
      ...
      <footer>
        ...
        <div class="flex gap-5 mt-4">
          ...
          >
            ...
            {{ topicStore.topic.numberComments }}
          </button>
        </div>
      </footer>
    </article>
    ...
    <div class="flex flex-row items-center justify-between px-2 py-4">
      <div class="inline-flex flex-row w-full gap-4">
        <a
          ...
          @click="topicStore.setOrderBy('mais recentes')"
          ><p :class="{ 'text-c-blue-500  ': topicStore.orderBy == 'mais recentes' }">
            Mais recentes
          </p></a
        >

        <a class="text-sm cursor-pointer text-c-gray-500" @click="topicStore.setOrderBy('melhores')"
          ><p :class="{ 'text-c-blue-500  ': topicStore.orderBy == 'melhores' }">Melhores</p></a
        >
      </div>
    </div>
  </div>
  ...
  <!-- Comments -->
  <Comment v-for="comment in topicStore.comments" :key="comment.id" :comment="comment" />
</template>
...
const formattedActivity = computed(() => {
  const diff = Date.now() - new Date(topicStore.topic.updatedAt).getTime();
